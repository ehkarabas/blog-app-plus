...
module.exports = (req, res, next) => {
  ...
  // * Find query'si populate'ten once calisir, yapisi da chain oldugundan bu mantikli gorunecektir. Yani await Model.find({ userId: idVal }).populate("userId"); seklinde bir kullanim hata vermez, once find query'si doner, sonra populate edilir.
  ...
  res.getModelListDetails = async (Model, condition = {}) => {
    // console.log("🔭 ~ getModelListDetails condition ➡ ➡ ", condition);
    // 🔭 ~ getModelListDetails condition ➡ ➡  { status: 'Published', destroyTime: null }
